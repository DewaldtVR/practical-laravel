<template>
  <div>
    <v-list dense>
      <small v-if="label" class="grey--text pl-4">{{ label }}</small>
      ...
      <v-list-tile v-for="(file,index) in fileList" :key="index" avatar>
        ...
        <v-list-tile-content>
          <v-list-tile-title>
            <v-layout row wrap>
              <v-flex xs6 >{{ file.originalfilename }}</v-flex>
              <v-flex xs6 >{{ getCreatedDate(file) }}</v-flex>
            </v-layout>
          </v-list-tile-title>
      ...
      <template v-if="fileList.length!==maxFiles" two-line>
...
export default {
  ...
  methods: {
    getCreatedDate(file){
      return this.$formatDate(file.created_at, 'D MMM YYYY HH:mm a');
    },
    addFile() {
      if (this.id !== null) {
