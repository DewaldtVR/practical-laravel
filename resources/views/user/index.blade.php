...
@section('content')
    <grid url="{{route('users.list')}}" title="User list" mode="addEdit" :row-menu="rowMenu" @config="catchGridConfig($event)">
    ...
    <modal-custom v-model="roleDialog" title="Roles" color="primary" width="600">
        <v-layout row wrap slot="content">
            ...
            <v-flex xs12 sm6 md6 >
                <v-radio-group v-model="userHasRoles">
                    <v-radio
    ...
    <script type="text/javascript">
        Laravel.vueMixins.push({
            ...
            methods: {
                ...
                openInNewTab(){
                    window.open('users/extractusers', '_blank').focus()
                },
                async activateUser(userid) {
                    let answer = await this.$dialog("You are about to deactivate this user profile", "Are you sure you want to do this?", "Success", null, "gavel", "25%", true, {
                        yes: {
                            text: "Yes",
                            color: "black"
                        },
                    ...
                    this.loading = true;
                    if (answer === "yes") {
                         this.$http.post('/users/' + userid + '/deactivate', $userid = userid).then((response) => {
                            this.loading = false;
                            if (response.status === 200) {
                                this.$snackbar("User has been deactivated succesfully", "success", "check", "bottom", "right", 3000)
                                this.refreshGrid()
                            } else {
                            ...
                            this.loading = false
                            this.$snackbar("You cannot deactivate yourself", "error", "error", "bottom", "right", 3000)
                            console.log(error)
                        });
                    }
                },
